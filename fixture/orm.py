...
#привязка строится в виде набора классов
    ...
    def get_contact_list(self):  #реализуем функции, кот. получают списки объектов
        return self.convert_contacts_to_model(select(c for c in ORMFixture.ORMContact if c.deprecated is None)) #запрос извлекает все контакты
    ...
    @db_session
    def get_contacts_not_in_group(self, group): #получаем список контактов, которые не входят в заданную группу
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == group.id))[0]
